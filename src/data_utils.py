...
def get_datasets() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    Returns: app_train : pd.DataFrame
        Training dataset
    ...
    if not os.path.exists(config.DATASET_DESCRIPTION):
        gdown.cached_download(
            config.DATASET_DESCRIPTION_URL,
            config.DATASET_DESCRIPTION,
            quiet=False,
            path_to_gdown_cache=certifi.where(),
        )

    if not os.path.exists(config.DATASET_TEST):
        gdown.cached_download(
            config.DATASET_TEST_URL,
            config.DATASET_TEST,
            quiet=False,
            path_to_gdown_cache=certifi.where(),
        )
...
def get_feature_target(
    ...
    X_train = app_train.drop(
        "TARGET", axis=1
    )  # Dropping the 'TARGET' column for features
    y_train = app_train[
        "TARGET"
    ]  # Selecting only the 'TARGET' column for the target variable

    # Extracting features and target variable for test set
    X_test = app_test.drop(
        "TARGET", axis=1
    )  # Dropping the 'TARGET' column for features
    y_test = app_test[
        "TARGET"
